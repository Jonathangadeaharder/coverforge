...
export type Accessory = "keine" | "brille" | "sonnenbrille" | "hut" | "muetze" | "bart";
...
export type ClothingStyle = "tshirt" | "hoodie" | "jacket" | "dress" | "sweater" | "collared";
...
export type Prop =
  ...
  | "camera"
  | "speech";
export type SceneElement = "none" | "skyline" | "mountains" | "stars" | "clouds" | "confetti";
...
const ACCESSORIES: Accessory[] = ["keine", "brille", "sonnenbrille", "hut", "muetze", "bart"];

const TYPOGRAPHY: TypographyStyle[] = ["blockbuster", "prestige", "scifi", "arthouse"];

const EXPRESSIONS: Expression[] = ["neutral", "smile", "serious", "surprised", "wink"];
...
const CLOTHING_PATTERNS: ClothingPattern[] = ["solid", "stripes", "dots", "emblem"];
...
export function buildRecipe(hash: string, text = ""): Recipe {
  ...
  if (/\b(frau|maedchen|mädchen|woman|girl|she|her|sie|lola|bridget|sacha|annie|hind|halima)\b/.test(lowerText)) {
    gender = "weiblich";
  } else if (/\b(mann|junge|man|boy|he|him|his|er|nico|hector|sam|pablo|david|sweden)\b/.test(lowerText)) {
    gender = "maennlich";
  }

  // 2. Age
  let age = getAttr("Alter", 1, AGES);
  if (/\b(kind|youth|child|kid|boy|girl|junge|maedchen|mädchen)\b/.test(lowerText)) {
    age = "kind";
  } else if (/\b(alt|greis|old|aging|chemistry teacher|babben|concierge)\b/.test(lowerText)) {
    age = "alt";
  } else if (/\b(junger|junge|adult|erwachsen|man|woman|mann|frau|colocataires|colleagues|friends)\b/.test(lowerText)) {
    age = "erwachsen";
  }
  ...
  if (/\b(muskuloes|muskulös|muscular|ken|strong|athletic)\b/.test(lowerText)) {
    body = "muskuloes";
  } else if (/\b(dick|kraeftig|kräftig|fat|chubby|large|stout)\b/.test(lowerText)) {
    body = "dick";
  ...
  } else if (/\b(schwarz|black|dark|dunkel|dunkle|spanier|moroccan|marokko|arab|darija)\b/.test(lowerText)) {
  ...
  } else if (/\b(komödie|comedy|funny|sun|warm|spain|spanien|spanisch)\b/.test(lowerText)) {
    palette = PALETTES.find((p) => p.name === "Sonnen-Komödie") || palette;
  } else if (/\b(indie|pastell|pastel|romance|love|liebe|french|französisch|english|englisch)\b/.test(lowerText)) {
    palette = PALETTES.find((p) => p.name === "Pastell Indie") || palette;
  } else if (/\b(forest|wald|nature|grün|green|schwarzwald)\b/.test(lowerText)) {
    palette = PALETTES.find((p) => p.name === "Waldschatten") || palette;
  } else if (/\b(blood|sunset|red|romantik|marokko|moroc|darija|drama)\b/.test(lowerText)) {
    palette = PALETTES.find((p) => p.name === "Blut-Sonnenuntergang") || palette;
  } else if (/\b(royal|prestige|crown|gold|throne|game|sweden|schwedisch)\b/.test(lowerText)) {
    palette = PALETTES.find((p) => p.name === "Royal Prestige") || palette;
  }

  // 8. Typography
  let typography = getAttr("Typografie", 10, TYPOGRAPHY);
  if (/\b(cyberpunk|scifi|science fiction|tech|computer|hacker)\b/.test(lowerText)) {
    typography = "scifi";
  } else if (/\b(action|thriller|crime|blockbuster|action-komödie)\b/.test(lowerText)) {
    typography = "blockbuster";
  } else if (/\b(indie|arthouse|documentary|independ|pastell|komödie|comedy)\b/.test(lowerText)) {
    typography = "arthouse";
  } else if (/\b(drama|romance|history|epische|telenovela|prestige|liebe|betrug|schicksal)\b/.test(lowerText)) {
    typography = "prestige";
  }

  // 9. Expression (byte 11)
  let expression = EXPRESSIONS[seg(hash, 11) % EXPRESSIONS.length];
  if (/\b(lachen|lacht|smile|happy|glücklich|fröhlich|funny|comedy)\b/.test(lowerText)) {
    expression = "smile";
  } else if (/\b(ernst|serious|streng|düster|noir|thriller)\b/.test(lowerText)) {
    expression = "serious";
  } else if (/\b(überrascht|surprised|shocked|shocked|schock)\b/.test(lowerText)) {
  ...
  let clothingPattern = CLOTHING_PATTERNS[seg(hash, 13) % CLOTHING_PATTERNS.length];
  ...
  if (/\b(buch|book|lesen|reading|library)\b/.test(lowerText)) {
    ...
    prop = "coffee";
  } else if (/\b(kopfhörer|headphones|earphones|musik hören)\b/.test(lowerText)) {
    prop = "headphones";
  ...
  if (/\b(skyline|city|stadt|urban|metro)\b/.test(lowerText)) {
    ...
    sceneElement = "clouds";
  } else if (/\b(konfetti|confetti|party|celebration|feier)\b/.test(lowerText)) {
...
export function buildRecipeWithSteps(hash: string, text = ""): DeterministicResult {
  ...
  const defaultGender = GENDERS[parseInt(hash.substring(0, 2), 16) % GENDERS.length];
  addStep("Geschlecht", recipe.gender !== defaultGender, recipe.gender, 0, GENDERS);

  const defaultAge = AGES[parseInt(hash.substring(2, 4), 16) % AGES.length];
  addStep("Alter", recipe.age !== defaultAge, recipe.age, 1, AGES);

  const defaultBody = BODIES[parseInt(hash.substring(4, 6), 16) % BODIES.length];
  ...
  addStep("Accessoire", recipe.accessory !== ACCESSORIES[seg(hash, 7) % ACCESSORIES.length], recipe.accessory, 7, ACCESSORIES);
  addStep("Palette", false, recipe.palette.name, 9, PALETTES.map((p) => p.name));
  ...
  addStep("Kleidungsmuster", false, recipe.clothingPattern, 13, CLOTHING_PATTERNS);
