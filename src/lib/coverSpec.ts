...
const characterSchema = z.strictObject({
  ...
  hairStyle: z
    .enum(["bald", "short", "mohawk", "long", "ponytail", "bun", "afro", "braids"])
    .optional(),
  ...
  clothingStyle: z
    ...
    .optional(),
  clothingPattern: z
    .enum(["solid", "stripes", "dots", "emblem"])
    .optional(),
  prop: z
    .enum([
      "none",
...
function resolvePalette(
  ...
  const idx = parseInt(hash.substring(byteIndex * 2, byteIndex * 2 + 2), 16) || 0;
...
async function characterRecipe(
  ...
  if (c.clothingPattern) r.clothingPattern = CLOTHING_PATTERN_MAP[c.clothingPattern];
...
export async function coverSpecToPlan(spec: CoverSpec): Promise<CoverSpecPlan> {
  ...
  if (mood.sceneElement) main.sceneElement = SCENE_ELEMENT_MAP[mood.sceneElement];
