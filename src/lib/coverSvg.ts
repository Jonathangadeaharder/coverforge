...
function lighten(hex: string, amt = 0.2): string {
	...
	const r = Math.min(
		255,
		Math.round(
			parseInt(n.substr(0, 2), 16) + (255 - parseInt(n.substr(0, 2), 16)) * amt,
		),
	);
	const g = Math.min(
		255,
		Math.round(
			parseInt(n.substr(2, 2), 16) + (255 - parseInt(n.substr(2, 2), 16)) * amt,
		),
	);
	const b = Math.min(
		255,
		Math.round(
			parseInt(n.substr(4, 2), 16) + (255 - parseInt(n.substr(4, 2), 16)) * amt,
		),
	);
	return `#${[r, g, b].map((v) => v.toString(16).padStart(2, "0")).join("")}`;
}
	...
	if (sc === "confetti") {
		...
		for (let i = 0; i < 24; i++) {
			...
			if (i % 3 === 0) {
				...
				pieces.push(
					`<circle cx="${cx2}" cy="${cy2}" r="5" fill="${col}" opacity="0.75"/>`,
				);
			} else {
				pieces.push(
...
function face(r: Recipe): string {
	...
	if (hashNibble === 0) {
		// Standard oval
		parts.push(
			`<ellipse cx="${CX}" cy="${d.cy}" rx="${d.rx}" ry="${d.ry}" fill="${skin}"/>`,
		);
	} else if (hashNibble === 1) {
		// Rounder
		parts.push(
			`<ellipse cx="${CX}" cy="${d.cy}" rx="${d.rx + 8}" ry="${d.ry - 6}" fill="${skin}"/>`,
		);
	} else {
...
function splitTitle(title: string): string[] {
	...
	for (let i = 0; i < title.length; i++) {
		if (
			title[i] === " " &&
			(best === -1 || Math.abs(i - mid) < Math.abs(best - mid))
		)
			best = i;
	}
...
function titleLayer(r: Recipe, raw: string): string {
	...
	const tspans = lines
		.map(
			(l, i) => `<tspan x="${CX}" y="${startY + i * lineH}">${esc(l)}</tspan>`,
		)
		.join("");
...
function buildCharacterGroup(r: Recipe): string {
	return (
		hairBack(r) + clothing(r) + face(r) + hairFront(r) + accessory(r) + prop(r)
	);
}
...
function layoutSlots(count: number): CharSlot[] {
	...
	return [
		{ cx: 180, scale: 0.7, zIndex: 0 },
		{ cx: 400, scale: 0.78, zIndex: 2 },
		{ cx: 620, scale: 0.7, zIndex: 1 },
	];
}
...
export function buildMultiCoverSVG(
	...
	const sorted = slots
		.map((slot, i) => ({
			slot,
			recipe: characterRecipes[i] ?? characterRecipes[0],
		}))
