...
import { renderCoverFn } from "@/lib/coverServer";
...
export function CoverGenerator() {
  ...
  const [hashResult, setHashResult] = useState<DeterministicResult | null>(
    null,
  );
  ...
  // Typst rendering state
  const [typstSvg, setTypstSvg] = useState<string>("");
  const [isCompilingTypst, setIsCompilingTypst] = useState(false);
  ...
  const activeRecipe = isManual
    ? manualRecipe
    : (hashResult?.recipe ?? manualRecipe);
  ...
  // Compile cover using Typst on the server when recipe/characters/title changes
  useEffect(() => {
    if (!activeRecipe) return;
    let active = true;
    const compile = async () => {
      ...
      try {
        ...
        if (active && result.success && result.svg) {
          setTypstSvg(result.svg);
        }
      } catch (err) {
        console.error("Failed to compile Typst cover on server:", err);
      } finally {
        if (active) {
    ...
    const timer = setTimeout(compile, 450);
    return () => {
      active = false;
      clearTimeout(timer);
    };
  ...
  const displaySvg = typstSvg || fallbackSvg;
  ...
  const copySvgCode = () => {
    if (!displaySvg) return;
    navigator.clipboard.writeText(displaySvg);
    setCopiedSvg(true);
                  ...
                  <div key={key}>
                    <div className="mb-1 flex justify-between text-xs font-semibold">
                      ...
                      <span className="font-mono text-foreground">
                        {MANUAL_LABELS[key]?.[String(manualRecipe[key])] ??
                          String(manualRecipe[key])}
                      </span>
                    </div>
        ...
        {activeRecipe && (
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              <Chip
                label="Geschlecht"
                value={GENDER_LABEL[activeRecipe.gender]}
              />
              <Chip label="Alter" value={AGE_LABEL[activeRecipe.age]} />
              <Chip label="Statur" value={BODY_LABEL[activeRecipe.body]} />
              <Chip
                label="Accessoire"
                value={ACC_LABEL[activeRecipe.accessory]}
              />
              <Chip
                label="Typografie"
                value={TYPO_LABEL[activeRecipe.typography]}
              />
      ...
      <div className="order-1 lg:order-2">
        <div className="sticky top-6">
          ...
          <div className="relative">
            ...
            />
            {isCompilingTypst && (
              <div className="absolute top-3 right-3 flex items-center gap-1.5 rounded-full border border-primary/20 bg-background/80 px-2.5 py-1 text-[10px] font-semibold text-primary backdrop-blur">
          ...
          <p className="mt-3 text-center text-xs text-muted-foreground">
            Vektor · 800 × 1200 · skalierbar · identischer Text = identisches
            Cover
          </p>
        </div>
      </div>
